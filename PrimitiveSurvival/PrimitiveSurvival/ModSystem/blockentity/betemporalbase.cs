namespace PrimitiveSurvival.ModSystem
{
    ...
    public class BETemporalBase : BlockEntityDisplayCase
    {
        ...
        private void GenerateParticles(IAsyncParticleManager manager)
        {
            ...
            {
                //cthulu defaults
                var color = ColorUtil.ToRgba(80, 50, 120, 50);
                var gravityEffect = 0.5f;

                if (topType == "dagon" || topType == "hydra")
                {
                    color = ColorUtil.ToRgba(80, 30, 30, 30);
                    gravityEffect = 1.0f;
                }
                ...
                particles.MinPos.Set(this.Pos.ToVec3d().AddCopy(-(gearCount - 1), 0f, -(gearCount - 1)));
                particles.AddPos.Set(new Vec3d(gearCount + gearCount - 1, 0.2f, gearCount + gearCount - 1));
