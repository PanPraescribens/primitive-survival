namespace PrimitiveSurvival.ModSystem
{
    ...
    public class BESnare : BlockEntityDisplayCase, IAnimalFoodSource
    {
        ...
        public override void GetBlockInfo(IPlayer forPlayer, StringBuilder sb)
        {
            ...
            { sb.Append(Lang.Get("blockdesc-snare-tripped")); }
            else
            {
                if (!this.BaitSlot.Empty)
                {
                    ...
                    {
                        if (Array.IndexOf(this.baitTypes, this.BaitStack.Item.FirstCodePart()) < 0)
                        { sb.Append(Lang.Get("blockdesc-snare-bait-rotten")); }
                        else
                        { sb.Append(Lang.Get("blockdesc-snare-bait-ok")); }
                    }
                    else if (this.BaitStack.Block != null)
                    {
                        if (Array.IndexOf(this.baitTypes, this.BaitStack.Block.FirstCodePart()) < 0)
                ...
                { sb.Append(Lang.Get("blockdesc-snare-bait-needed")); }
            }
