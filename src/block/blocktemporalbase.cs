...
public class BlockTemporalBase : Block
{
    public MeshData GenMesh(ICoreClientAPI capi, string shapePath, ITexPositionSource texture, int index, string type, string dir, ITesselatorAPI tesselator = null)
    {
        ...
        /*else if (shapePath.Contains("necronomicon-open"))
        {
    ...
    {
        BETemporalBase be = world.BlockAccessor.GetBlockEntity(pos) as BETemporalBase;
    ...
    {
        BETemporalBase be = world.BlockAccessor.GetBlockEntity(blockSel.Position) as BETemporalBase;
        if (be != null)
