...
public class BlockRopeBridge : Block
{
    public virtual int GetRopeBridgeLength(IBlockAccessor blockAccessor, BlockPos pos, string facing)
    {
        int length = 0;
    ...
    public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
    {
        ...
        int lineLength = GetRopeBridgeLength(world.BlockAccessor, blockSel.Position.Copy(), facing);
        if (itemstack.StackSize <= (lineLength - 2))
        {
            failureCode = "you will need more rope bridge to span this area";
            return false;
        }
        if (lineLength > 1)
        {
        ...
        {
            failureCode = "you need suitable ground at both ends of the rope bridge and a clear line of sight";
            return false;
        }
    }
    ...
    public override void OnNeighbourBlockChange(IWorldAccessor world, BlockPos pos, BlockPos neibpos)
    {
        ...
        {
            if (!CanRopeBridgeStay(world.BlockAccessor, pos))
            {
    ...
    public virtual bool CanRopeBridgeStay(IBlockAccessor blockAccessor, BlockPos pos)
    {
        Block block = blockAccessor.GetBlock(pos.X, pos.Y - 1, pos.Z);
        return block.Fertility > 0;
    }

    public virtual int removeRopeBridge(IWorldAccessor world, BlockPos pos, string dir, bool fromMiddle, int mult)
    {
        int count = 1;
        ...
        do
        {
            ...
            if (bBlock.FirstCodePart() == "ropebridge")
            {
                if (bBlock.FirstCodePart(1) == "farend" || bBlock.FirstCodePart(1) == "nearend")
                {
                    thisDirDone = true;
    ...
    public override void OnBlockBroken(IWorldAccessor world, BlockPos pos, IPlayer byPlayer, float dropQuantityMultiplier = 1)
    {
        ...
        string type = FirstCodePart(1);
        int removed = 0;
        bool fromMiddle = false;
        if (type != "farend" && type == "nearend")
        { fromMiddle = true; }
        removed += removeRopeBridge(world, pos, dir, fromMiddle, 1);
        removed += removeRopeBridge(world, pos, dir, fromMiddle, -1);
        removed -= 3;
