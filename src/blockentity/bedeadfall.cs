...
public class BEDeadfall : BlockEntityDisplay, IAnimalFoodSource
{
    ...
    {
        BlockDeadfall block = Api.World.BlockAccessor.GetBlock(Pos) as BlockDeadfall;
        ItemStack stack = baitSlot.TakeOut(1);
        if (stack != null)
        {
            Api.World.SpawnItemEntity(stack, Pos.ToVec3d().Add(0.5, 0.5, 0.5));
        ...
        block = Api.World.BlockAccessor.GetBlock(block.CodeWithPath(blockPath)) as BlockDeadfall;
